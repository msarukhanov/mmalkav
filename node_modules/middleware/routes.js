...
module.exports = function (app, fs) {
    ...
    //=== load one file with regions ===
     app.get('/getProvinces', function (req, res) {
         fs.readFile('./data/default/regions.json', 'utf8', function (err, data) {
             res.type('application/json');
             res.jsonp(data);
         });
     });
    ...
    app.get('/getCampaignInfo', function (req, res) {
        try {
            var query = region_ownersSchema.find({})
                .exec(function (err, docs) {
                    if (!err) {
                        res.type('application/json');
                        res.jsonp(docs);
    ...
    app.post('/newCampaignInfo', function (req, res) {
        try {
           region_ownersSchema.collection.insert({
               "id" : req.body.regionID,
               "owner" : req.body.new_owner
           }, function (err, docs) {
                if (err) {
                    console.log(err);
                } else {
                    console.log('region_new_owner successfully');
                    res.type('application/json');
                    res.jsonp({});
                }
            });
    ...
    app.post('/resetCampaignInfo', function (req, res) {
       fs.readFile('./data/default/campaignInfo.json', 'utf8', function (err, data) {
           ...
           Object.keys(map).forEach(function(id) {
               ...
               try {
                   var query = region_ownersSchema.collection.update({id: id}, {
                       $set: {owner: map[id]}
                   }, {}, function (err, docs) {
                       if (err) {
                           console.log(err);
